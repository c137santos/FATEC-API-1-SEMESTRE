...
def criacao_turma(dados_nova_turma, grupos):
    ...
    if (
        len(dados_nova_turma_json["grupos"]) >= 1
    ):  # Se a lista vier com pelo menos 1 grupo
        for idGrupo in dados_nova_turma_json[
            "grupos"
        ]:  # Percorre a lista para pegar cada id
            idGrupo = (
                str(  # Transforma o id em str para pode ser comparado com o grupos.json
                    idGrupo
                )
            )
            ...
            if idGrupo in grupos:  # pega esse id da lista e procura ele no grupos.json
                ...
                grupos[idGrupo]["turma"] = int(
                    nova_turma_id
                )  # Atualize a propriedade "turma" do grupo com um valor inteiro
                ...
                """Nessa resposta por exemplo: pega o nome do grupo e o nome da turma 
                para dizer que foi feito com sucesso a operacao"""
            else:
                resposta["detalhes"].append(
